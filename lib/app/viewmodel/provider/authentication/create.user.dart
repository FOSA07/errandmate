...
class CreateUserAccountNotifier extends _$CreateUserAccountNotifier with GlobalDialog, ViewRouter{

  Future createUser () async {
    try{
      state = const AsyncLoading();
      CreateUserModel userModel = ref.read(createUserFormNotifierProvider);
      final response = await CreateUserController().createUser(userModel: userModel);
      ...
      response.fold(
        ...
        (result) {
          ...
          goto(
            '/auth/otp/$tk/${result["data"]["userid"]}'
          );
        }
      );
    }catch(e){
